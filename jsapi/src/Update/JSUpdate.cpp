    ...
    void download(JQAsyncInfo& info) {
        try {
            ...
            if (!checkResponse.isOk()) {
                info.postError("Failed to check for updates: HTTP " + std::to_string(checkResponse.status));
                return;
            }
            ...
            if (downloadUrl.empty()) {
                info.postError("No matching download asset found");
                return;
            }
            ...
            if (!downloadResponse.isOk()) {
                info.postError("Download failed: HTTP " + std::to_string(downloadResponse.status));
                return;
            }
            ...
            std::ofstream file(savePath, std::ios::binary);
            if (!file) {
                info.postError("Failed to create file: " + savePath);
                return;
            }
...
// 包装函数以适配SetProtoMethod模板
static void js_setRepo(JQFunctionInfo& info) {
    JSUpdate* self = static_cast<JSUpdate*>(info.GetData());
    if (self) {
        self->setRepo(info);
    }
}

static void js_check(JQFunctionInfo& info) {
    JSUpdate* self = static_cast<JSUpdate*>(info.GetData());
    if (self) {
        self->check(info);
    }
...
static void js_download(JQAsyncInfo& info) {
    JSUpdate* self = static_cast<JSUpdate*>(info.GetData());
    if (self) {
        self->download(info);
    }
...
static void js_getConfig(JQFunctionInfo& info) {
    JSUpdate* self = static_cast<JSUpdate*>(info.GetData());
    if (self) {
        self->getConfig(info);
    }
...
static void js_cleanup(JQFunctionInfo& info) {
    JSUpdate* self = static_cast<JSUpdate*>(info.GetData());
    if (self) {
        self->cleanup(info);
    }
...
JSValue createUpdate(JQModuleEnv* env)
{
    ...
    tpl->SetProtoMethod("getConfig", &JSUpdate::getConfig);
