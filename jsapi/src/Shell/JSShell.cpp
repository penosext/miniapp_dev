...
bool Shell::isInteractiveProgram(const std::string& program) {
    ...
    return std::find(interactivePrograms.begin(), interactivePrograms.end(), 
                     program) != interactivePrograms.end();
}

Shell::CommandResult Shell::execInteractiveProgram(const std::string& program, 
                                                   const std::vector<std::string>& args) {
    CommandResult result;
    
    if (!isInteractiveProgram(program)) {
        result.error = program + " is not recognized as an interactive program";
        return result;
    }
    ...
    // 对于交互式程序，需要使用交互式Shell
    if (impl->config.type != SHELL_INTERACTIVE) {
        result.error = "Interactive programs require interactive shell mode";
        return result;
    }
    
    if (!impl->ensureRunning()) {
        result.error = "Failed to start interactive shell";
        return result;
    }
    
    // 写入命令
    if (!impl->writeToShell(cmd + "\n")) {
        result.error = "Failed to start interactive program";
        return result;
    }
    
    // 标记为等待输入状态
    impl->state = STATE_WAITING_INPUT;
    if (impl->stateCallback) {
        impl->stateCallback(impl->state);
    }
